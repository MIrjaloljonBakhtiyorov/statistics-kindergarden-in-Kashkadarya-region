...
const initializeSchema = () => {
    db.serialize(() => {
        ...
        WHERE login IS NOT NULL AND trim(login) <> ''`, (err) => {
            if (err)
                console.error('Migration error on parent login unique index:', err.message);
        });
        ...
        WHERE username IS NOT NULL AND trim(username) <> ''`, (err) => {
            if (err)
                console.error('Migration error on kindergarten username unique index:', err.message);
        });
        ...
        WHERE system_id IS NOT NULL AND trim(system_id) <> ''`, (err) => {
            if (err)
                console.error('Migration error on kindergarten system_id unique index:', err.message);
        });
        const createIndex = (name, table, columns) => {
            db.run(`CREATE INDEX IF NOT EXISTS ${name} ON ${table} (${columns})`, (err) => {
                if (err)
                    console.error(`Migration error on index ${name}:`, err.message);
            });
        };
        const ensureCascadeForeignKey = (table, constraint, column, referenceTable, referenceColumn = 'id') => {
            ...
            db.run(`ALTER TABLE ${table} ADD CONSTRAINT ${constraint} FOREIGN KEY (${column}) REFERENCES ${referenceTable}(${referenceColumn}) ON DELETE CASCADE`, (addErr) => {
        ...
        db.run('DROP FUNCTION IF EXISTS prevent_aqlvoy_dish_delete()', (err) => {
            if (err)
                console.error('Migration error dropping Aqlvoy dish delete function:', err.message);
        });
