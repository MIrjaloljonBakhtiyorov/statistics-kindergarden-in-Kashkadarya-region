...
import { decorateMedicalItems, ensureMedicalInventoryDefaults, ensureTables, normalizeMedicineName, } from '../../kindergarten/routes/routeSupport.js';
...
const recordAdminAlertEvent = async ({ eventType, category, status, title, context, actor = 'Admin', entityType, entityId, actionUrl, details = [], }) => {
    await ensureAdminAlertEventsTable();
    const id = crypto.randomUUID();
    await run(`INSERT INTO admin_alert_events (
...
const callOpenAIInsights = async (snapshot) => {
    ...
    if (!response.ok)
        throw new Error(data?.error?.message || 'OpenAI analysis failed');
...
const buildMedicalStockReport = async () => {
    await ensureTables();
    const kindergartens = await all(`SELECT id, name, district, address, phone, currentChildren
     FROM kindergartens
     ORDER BY district, name`);
    await Promise.all(kindergartens.map((kindergarten) => ensureMedicalInventoryDefaults(String(kindergarten.id))));
    ...
    const rawItems = rows.map((row) => {
        ...
        if (Number(decorated.movement_count || 0) === 0)
            status = 'NOT_ENTERED';
        else if (expiredBatchCount > 0)
            status = 'EXPIRED';
        else if (Number(decorated.current_quantity || 0) <= 0)
            status = 'EMPTY';
        else if (decorated.nearest_expiry_date && decorated.nearest_expiry_date <= warningIso)
            status = 'EXPIRING';
        else if (Number(decorated.required_quantity || 0) > 0 && Number(decorated.current_quantity || 0) < Number(decorated.required_quantity || 0))
            status = 'LOW';
        return {
            ...decorated,
            status,
...
const generateUniqueUsername = async (name) => {
    ...
    for (let index = 0; index < 1000; index += 1) {
        ...
        const existing = await get('SELECT id FROM kindergartens WHERE lower(trim(username)) = lower(trim(?))', [candidate]);
        if (!existing)
            return candidate;
    }
...
const generateKindergartenCredentials = async (name) => {
  ...
  `);
    ...
    for (let attempt = 0; attempt < 1000; attempt += 1) {
        ...
        const existing = await get(`SELECT id FROM kindergartens
       WHERE system_id = ?
          OR lower(trim(username)) = lower(trim(?))`, [systemId, username]);
        if (!existing) {
...
const KindergartenController = {
    ...
    getInspections: async (req, res) => {
        try {
          ...
          OR LOWER(COALESCE(act_number, '')) LIKE ?
            ...
            const rows = await all(`SELECT * FROM admin_kindergarten_inspections
         ${where}
         ORDER BY inspection_date DESC, created_at DESC
         LIMIT 400`, params);
            const monthlySummary = await all(`SELECT
         ...
         ORDER BY entries DESC, district ASC`, [selectedMonth]);
            const availableMonths = await all(`SELECT SUBSTR(COALESCE(inspection_date, ''), 1, 7) as month, COUNT(*) as entries
         FROM admin_kindergarten_inspections
    ...
    downloadInspectionWord: async (req, res) => {
        try {
            ...
            if (!row)
                return res.status(404).json({ error: 'Dalolatnoma topilmadi' });
    ...
    createInspection: async (req, res) => {
        try {
            ...
            if (!kindergartenId)
                return res.status(400).json({ error: 'Bogcha tanlanishi shart' });
            if (!inspectorFullName)
                return res.status(400).json({ error: 'Inspektor F.I.Sh kiritilishi shart' });
            ...
            const kindergarten = await get(`SELECT id, name, region, district FROM kindergartens WHERE CAST(id AS TEXT) = CAST(? AS TEXT)`, [kindergartenId]);
            if (!kindergarten)
                return res.status(404).json({ error: 'Bogcha topilmadi' });
            const id = crypto.randomUUID();
            ...
            await run(`INSERT INTO admin_kindergarten_inspections (
    ...
    getDailyDistrictExpenses: async (req, res) => {
        ...
        try {
            const result = await db.pool.query(`SELECT id, date, district, cost_per_child, note, created_at, updated_at
         FROM daily_district_expenses
         WHERE date = $1
    ...
    saveDailyDistrictExpenses: async (req, res) => {
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                const result = await client.query(`INSERT INTO daily_district_expenses (id, date, district, cost_per_child, note, updated_at)
           VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP)
           ON CONFLICT (date, district)
           DO UPDATE SET cost_per_child = EXCLUDED.cost_per_child,
                         note = EXCLUDED.note,
                         updated_at = CURRENT_TIMESTAMP
           RETURNING id, date, district, cost_per_child, note, created_at, updated_at`, [id, date, district, costPerChild, note]);
    ...
    getDistrictWarehousePurchases: async (req, res) => {
        ...
        try {
            ...
            const result = await db.pool.query(`SELECT id, date, district, product_name, unit, quantity, price_per_unit, note, created_at, updated_at
         FROM admin_warehouse_purchases
         ${whereSql}
         ORDER BY date, district, product_name`, params);
            res.json({
                date,
    ...
    saveDistrictWarehousePurchases: async (req, res) => {
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                const note = String(entry?.note || '').trim() || null;
                const id = entry?.id || crypto.randomUUID();
                const result = await client.query(`INSERT INTO admin_warehouse_purchases (id, date, district, product_name, unit, quantity, price_per_unit, note, updated_at)
           ...
           DO UPDATE SET unit = EXCLUDED.unit,
                         ...
                         note = EXCLUDED.note,
                         updated_at = CURRENT_TIMESTAMP
           RETURNING id, date, district, product_name, unit, quantity, price_per_unit, note, created_at, updated_at`, [id, date, district, productName, unit, quantity, pricePerUnit, note]);
                saved.push(result.rows[0]);
            }
            await client.query('COMMIT');
            res.json({
    ...
    getAIInsights: async (req, res) => {
        ...
        try {
           ...
           WHERE cache_key = ? AND expires_at > CURRENT_TIMESTAMP`, [cacheKey]);
                if (cached) {
                    return res.json({
                        date: cached.date,
    ...
    getAlerts: async (req, res) => {
        ...
        `),
            ...
            const alerts = [...eventAlerts, ...medicalOperationAlerts, ...medicalAlerts, ...menuAlerts].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            const matchesFilter = (alert) => {
                if (filter === 'kindergarten')
                    return alert.category === 'kindergarten';
                if (filter === 'medical')
                    return alert.category === 'medical';
                if (filter === 'menu')
                    return alert.category === 'menu';
                if (filter === 'important')
                    return ['error', 'warning'].includes(alert.status);
                return true;
            };
    ...
    getMenuComplianceStatistics: async (req, res) => {
      ...
      `, [startDate, endDate]);
            const normalizeStatus = (row) => {
                ...
                if (/(qoidabuz|xato|rad|reject|failed|bad|nosoz|talabga javob bermaydi)/i.test(text))
                    return 'violation';
                if (/(taom|ovqat|o'rin|orin|boshqa|almashtirildi|meal)/i.test(comment))
                    return 'meal_replaced';
                if (/(mahsulot|ingredient|product|zaxira|yetib kelmagan|go'sht|gosht|baliq|sut|yog')/i.test(comment))
            ...
            menuRows.forEach((row) => {
                ...
                if (row.meal_name)
                    current.planned.push(row.meal_name);
                if (row.composition || row.products)
                    current.actual.push(row.composition || row.products);
                if (row.nurse_quality_comment)
                    current.comments.push(row.nurse_quality_comment);
                current.menus += 1;
    ...
    createTenDayMenus: (req, res) => {
        ...
        db.all(`SELECT id, workHours FROM kindergartens ${typeWhere}`, typeParams, (kindergartenErr, kindergartenRows = []) => {
            ...
            kindergartenRows.forEach((kindergarten) => {
                ...
                days.forEach((day) => {
                    Object.entries(day.meals || {}).forEach(([mealType, meal]) => {
                        ...
                        if (!mealData.meal_name || !String(mealData.meal_name).trim())
                            return;
                        menuRows.push({
            ...
            db.serialize(() => {
                ...
                db.all(`SELECT id FROM menus WHERE kindergarten_id IN (${targetPlaceholders}) AND date IN (${datePlaceholders})`, [...targetIds, ...dates], (selectErr, existingRows = []) => {
                    if (selectErr) {
                        db.run('ROLLBACK');
                    ...
                    const deleteMenus = () => {
                        db.run(`DELETE FROM menus WHERE kindergarten_id IN (${targetPlaceholders}) AND date IN (${datePlaceholders})`, [...targetIds, ...dates], (deleteErr) => {
                            if (deleteErr) {
                                db.run('ROLLBACK');
    ...
    delete: async (req, res) => {
        ...
        try {
            const existing = await get(`SELECT id, system_id, name, district, directorName as director_name, phone, capacity, username, created_at
         FROM kindergartens
