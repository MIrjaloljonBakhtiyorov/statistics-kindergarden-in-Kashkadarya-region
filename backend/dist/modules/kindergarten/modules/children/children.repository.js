...
export class ChildrenRepository {
    async create(data, kindergartenId) {
        ...
        try {
            ...
            await run('INSERT INTO parents (id, full_name, workplace, phone, passport_no, role, kindergarten_id) VALUES (?, ?, ?, ?, ?, ?, ?)', [fatherId, data.father_full_name, data.father_workplace, data.father_phone || null, data.father_passport, 'FATHER', kindergartenId]);
            await run('INSERT INTO parents (id, full_name, workplace, phone, passport_no, role, kindergarten_id) VALUES (?, ?, ?, ?, ?, ?, ?)', [motherId, data.mother_full_name, data.mother_workplace, data.mother_phone || null, data.mother_passport, 'MOTHER', kindergartenId]);
            await run('INSERT INTO parent_accounts (id, login, password_hash, kindergarten_id) VALUES (?, ?, ?, ?)', [accountId, finalLogin, passwordHash, kindergartenId]);
            await run(`
    ...
    async update(id, data, kindergartenId) {
        ...
        `);
                ...
                stmtChild.finalize();
                db.get('SELECT father_id, mother_id, parent_account_id FROM children WHERE id = ? AND kindergarten_id = ?', [id, kindergartenId], async (err, child) => {
                    if (err || !child) {
                        db.run('ROLLBACK');
