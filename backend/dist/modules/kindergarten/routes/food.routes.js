...
foodRoutes.post("/kitchen/tasks/:menuId/status", async (req, res) => {
    try {
        await ensureTables();
        const kindergartenId = await resolveKindergartenId(req);
        const existing = await get('SELECT id FROM kitchen_tasks WHERE kindergarten_id = ? AND menu_id = ?', [kindergartenId, req.params.menuId]);
        if (existing) {
            await run(`
...
foodRoutes.get("/lab/samples", async (req, res) => {
    try {
        await ensureTables();
        const kindergartenId = await resolveKindergartenId(req);
        const rows = await all('SELECT * FROM lab_samples WHERE kindergarten_id = ? ORDER BY timestamp DESC', [kindergartenId]);
        res.json(rows.map((row) => ({
            ...row,
