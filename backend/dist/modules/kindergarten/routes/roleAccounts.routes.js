...
import { all, get, run, ensureTables, } from './routeSupport.js';
...
roleAccountsRoutes.get("/role-accounts", async (req, res) => {
    try {
        ...
        const rows = await all('SELECT id, role, full_name, login, created_at, updated_at FROM role_accounts WHERE kindergarten_id = ? ORDER BY role', [kindergartenId]);
...
roleAccountsRoutes.put("/role-accounts/:role", async (req, res) => {
    try {
        ...
        if (!roleLabels[role])
            return res.status(400).json({ error: 'NotoКјgКјri rol' });
        const existing = await get('SELECT id FROM role_accounts WHERE kindergarten_id = ? AND role = ?', [kindergartenId, role]);
        ...
        await run(`INSERT INTO role_accounts (id, kindergarten_id, role, full_name, login, password_hash)
       VALUES (?, ?, ?, ?, ?, ?)`, [id, kindergartenId, role, fullName, login, await bcrypt.hash(req.body.password, 10)]);
