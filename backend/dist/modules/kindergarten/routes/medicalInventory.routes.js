...
import { all, get, run, ensureTables, toPositiveNumber, normalizeIsoDate, normalizeArchiveMonths, getArchiveCutoffDate, getKindergartenChildCount, ensureMedicalInventoryDefaults, getMedicalItemStock, decorateMedicalItems, logOperation, resolveMedicalOutDetails, } from './routeSupport.js';
...
medicalInventoryRoutes.post("/medical-inventory/items", async (req, res) => {
    try {
        ...
        if (!name)
            return res.status(400).json({ error: 'Dori nomi kiritilishi shart' });
...
medicalInventoryRoutes.post("/medical-inventory/movements", async (req, res) => {
    try {
        ...
        const item = await get('SELECT * FROM medical_inventory_items WHERE id = ? AND kindergarten_id = ?', [itemId, kindergartenId]);
        if (!item)
...
medicalInventoryRoutes.put("/medical-inventory/movements/:id", async (req, res) => {
    try {
        const kindergartenId = await resolveKindergartenId(req);
        const movement = await get('SELECT * FROM medical_inventory_movements WHERE id = ? AND kindergarten_id = ?', [req.params.id, kindergartenId]);
        if (!movement)
            return res.status(404).json({ error: 'Kirim/chiqim yozuvi topilmadi' });
