...
import { all, get, run, ensureTables, normalizeIsoDate, addMonthsToIsoDate, isHealthCheckDue, logOperation, } from './routeSupport.js';
...
healthExtraRoutes.post("/health/staff-check", async (req, res) => {
    try {
        ...
        if (!staffId)
            return res.status(400).json({ error: 'Xodim tanlanishi shart' });
        const staff = await get('SELECT id, full_name FROM staff WHERE id = ? AND kindergarten_id = ?', [staffId, kindergartenId]);
        if (!staff)
