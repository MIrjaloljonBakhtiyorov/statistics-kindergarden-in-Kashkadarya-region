...
export const resolveChatUserId = async (kindergartenId, userId, role) => {
    const normalizedRole = String(role || '').toUpperCase();
    const isKindergartenAccount = String(userId) === String(kindergartenId);
    if (!isKindergartenAccount && !['TEACHER', 'DIRECTOR'].includes(normalizedRole))
        return userId;
    const teacher = await get(`SELECT id FROM role_accounts
     WHERE kindergarten_id = ? AND role = 'TEACHER'
     ORDER BY created_at DESC
     LIMIT 1`, [kindergartenId]).catch(() => undefined);
        ...
        if (!promise) {
         ...
         WHERE role NOT IN (
           'OPERATOR', 'TEACHER', 'NURSE', 'CHEF', 'STOREKEEPER', 'INSPECTOR'
            ...
            promise = statements.reduce((chain, sql) => chain.then(() => run(sql).then(() => undefined)), Promise.resolve());
        }
        return promise;
    };
})();
...
export const normalizeMedicineName = (value) => String(value || '')
    .toLowerCase()
    .replace(/\u0432\u0402[\u0098\u0099\u2122]/g, "'")
    .replace(/[\u2018\u2019\u02bb\u02bc`]/g, "'")
    .replace(/[^a-z0-9]+/g, '')
...
export const normalizeIsoDate = (value, fallback = new Date().toISOString().slice(0, 10)) => {
    const date = String(value || '').slice(0, 10);
    return /^\d{4}-\d{2}-\d{2}$/.test(date) ? date : fallback;
};
export const addMonthsToIsoDate = (value, months) => {
    const date = String(value || '').slice(0, 10);
...
export const isHealthCheckDue = (latestDate, months) => {
...
export const normalizeArchiveMonths = (value) => {
    const months = Number(value || 24);
    return MEDICAL_ARCHIVE_MONTHS.includes(months) ? months : 24;
};
export const getArchiveCutoffDate = (months) => {
    const date = new Date();
    date.setMonth(date.getMonth() - months);
    return date.toISOString().slice(0, 10);
};
export const getKindergartenChildCount = async (kindergartenId) => {
    const children = await get('SELECT COUNT(*) as count FROM children WHERE kindergarten_id = ?', [kindergartenId]);
    const kindergarten = await get('SELECT currentChildren FROM kindergartens WHERE id = ?', [kindergartenId]);
    return Number(children?.count || kindergarten?.currentChildren || kindergarten?.currentchildren || 0);
};
export const ensureMedicalInventoryDefaults = async (kindergartenId) => {
    await ensureTables();
    const existingItems = await all('SELECT id, name FROM medical_inventory_items WHERE kindergarten_id = ?', [kindergartenId]);
    for (const item of FIRST_AID_MEDICINE_DEFAULTS) {
        ...
        if (!existing) {
            ...
            await run(`INSERT INTO medical_inventory_items
          (id, kindergarten_id, name, form, unit, required_per_100, required_label, is_default)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [id, kindergartenId, item.name, item.form, item.unit, item.required_per_100, item.required_label, 1]);
            existingItems.push({ id, name: item.name });
        }
    }
};
export const getMedicalItemStock = async (kindergartenId, itemId, excludeMovementId) => {
    const params = [kindergartenId, itemId];
    let excludeSql = '';
...
export const decorateMedicalItems = (rows, childCount) => {
    ...
    return rows.map((row) => {
        ...
        if (movementCount === 0)
            ...
            status = 'EMPTY';
        else if (nearestExpiryDate && nearestExpiryDate < today)
            status = 'EXPIRED';
        else if (nearestExpiryDate && nearestExpiryDate <= warningIso)
            status = 'EXPIRING';
        else if (requiredQuantity > 0 && currentQuantity < requiredQuantity)
            status = 'LOW';
        return {
            ...row,
...
export const logOperation = async (kindergartenId, operationType, entityType, entityName, description, category = 'OTHER') => {
    await run(`
...
export const resolveMedicalOutDetails = async (kindergartenId, body) => {
    ...
    const group = await get('SELECT id, name FROM groups WHERE id = ? AND kindergarten_id = ?', [groupId, kindergartenId]);
    if (!group)
        throw new Error('Guruh topilmadi');
    const child = await get(`SELECT id, first_name, last_name, group_id
     FROM children
     WHERE id = ? AND kindergarten_id = ?`, [childId, kindergartenId]);
    if (!child)
        throw new Error('Bola topilmadi');
