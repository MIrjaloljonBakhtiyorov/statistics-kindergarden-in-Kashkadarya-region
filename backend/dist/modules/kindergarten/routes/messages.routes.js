...
import { all, get, run, resolveChatUserId, } from './routeSupport.js';
...
messagesRoutes.get("/messages/contacts", async (req, res) => {
    ...
    `, [kindergartenId, parentId, parentId, parentId, kindergartenId]).catch(() => undefined);
        ...
        [...roleContacts, ...staffContacts, ...(director ? [director] : [])].forEach((row) => {
            if (!row?.id || contactsById.has(String(row.id)))
                return;
            contactsById.set(String(row.id), row);
        });
        res.json(Array.from(contactsById.values()).map((row) => ({
            id: String(row.id),
            name: row.full_name,
...
messagesRoutes.get("/messages/unread-counts", async (req, res) => {
    ...
    `, [kindergartenId, userId]);
        res.json(Object.fromEntries(rows.map((row) => [row.sender_id, row.count])));
    }
    catch (error) {
        res.status(500).json({ error: error.message });
    }
...
messagesRoutes.put("/messages/:id", async (req, res) => {
    try {
        ...
        const message = await get('SELECT * FROM messages WHERE id = ? AND kindergarten_id = ?', [req.params.id, kindergartenId]);
        if (!message)
        ...
        if (isDeletedMessage(message))
            return res.status(400).json({ error: 'O‘chirilgan xabarni tahrirlab bo‘lmaydi' });
        const editedAt = new Date().toISOString();
        await run('UPDATE messages SET text = ?, edited_at = ? WHERE id = ? AND kindergarten_id = ?', [text, editedAt, req.params.id, kindergartenId]);
...
messagesRoutes.delete("/messages/:id", async (req, res) => {
    try {
        ...
        if (!message)
            return res.status(404).json({ error: 'Xabar topilmadi' });
        if (message.sender_id !== userId)
            return res.status(403).json({ error: 'Faqat o‘z xabaringizni o‘chirishingiz mumkin' });
        const deletedAt = new Date().toISOString();
        await run(`UPDATE messages
       SET is_deleted = 1, deleted_at = ?, text = '', file_url = NULL, file_name = NULL, mime_type = NULL, message_type = 'text'
       WHERE id = ? AND kindergarten_id = ?`, [deletedAt, req.params.id, kindergartenId]);
        res.json(mapMessage({
            ...message,
            text: '',
            file_url: null,
