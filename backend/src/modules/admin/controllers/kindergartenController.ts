...
const buildAdminAIInsightSnapshot = async (date) => {
  ...
  const markAttendanceMetric = (metric, row) => {
    ...
    if (String(row.status || '').toUpperCase() === 'LATE' || (row.arrival_time && row.arrival_time > '09:30')) metric.late += 1;
  };
        ...
        days.forEach((day) => {
          Object.entries(day.meals || {}).forEach(([mealType, meal]) => {
            ...
            mealItems.forEach((mealItem) => {
              const mealData = typeof mealItem === 'string' ? { meal_name: mealItem } : (mealItem || {});
