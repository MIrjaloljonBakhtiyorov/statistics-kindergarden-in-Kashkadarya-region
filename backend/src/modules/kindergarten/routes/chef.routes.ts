...
const decorateSanitaryCheckpoint = (row: any) => ({
  ...row,
...
chefRoutes.get("/chef/sanitary-check/status/:chefId/:date", async (req, res) => {
  try {
    await ensureTables();
    const kindergartenId = await resolveKindergartenId(req);
    const row = await get<any>('SELECT passed FROM chef_sanitary_checks WHERE kindergarten_id = ? AND chef_id = ? AND date = ?', [
      kindergartenId,
      req.params.chefId,
      req.params.date,
    ]);
...
chefRoutes.post("/nurse/sanitary-checkpoints/:id/approve", async (req, res) => {
  try {
    ...
    if (!result.changes) return res.status(404).json({ error: 'Checkpoint topilmadi' });
...
chefRoutes.post("/nurse/meal-quality/:taskId", async (req, res) => {
  try {
    ...
    if (!result.changes) return res.status(404).json({ error: 'Taom vazifasi topilmadi' });
