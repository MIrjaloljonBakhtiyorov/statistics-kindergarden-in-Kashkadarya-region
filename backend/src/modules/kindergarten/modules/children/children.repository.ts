...
export class ChildrenRepository {
  ...
  async delete(id: string, kindergartenId: string): Promise<void> {
    return new Promise((resolve, reject) => {
      db.get('SELECT first_name, last_name FROM children WHERE id = ? AND kindergarten_id = ?', [id, kindergartenId], async (err, child: any) => {
        if (!child) return reject(new Error('Child not found'));
