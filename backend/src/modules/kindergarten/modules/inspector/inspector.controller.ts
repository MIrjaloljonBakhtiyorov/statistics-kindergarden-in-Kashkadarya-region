...
export class InspectorController {
  ...
  private update = async (req: any, res: any) => {
    try {
      ...
      `, [
        ...
        if (err) return res.status(500).json({ error: err.message });
        if (this.changes === 0) return res.status(404).json({ error: 'Inspection not found' });
  ...
  private submit = async (req: any, res: any) => {
    try {
      const kindergartenId = await resolveKindergartenId(req);
      db.run(
        `UPDATE audits SET status = 'COMPLETED', overall_result = COALESCE(overall_result, 'COMPLETED') WHERE id = ? AND kindergarten_id = ?`,
