...
const initializeSchema = () => {
    db.serialize(() => {
      ...
      db.run(`CREATE UNIQUE INDEX IF NOT EXISTS idx_users_email_nocase
        ON users (lower(trim(email)))
        WHERE email IS NOT NULL AND trim(email) <> ''`, (err) => {
        if (err) console.error('Migration error on users email unique index:', err.message);
      });
      ...
      createIndex('idx_email_codes_email_created', 'email_verification_codes', 'email, created_at DESC');

      db.run('DROP TRIGGER IF EXISTS protect_aqlvoy_dishes ON dishes', (err) => {
        if (err) console.error('Migration error dropping Aqlvoy dish delete trigger:', err.message);
      });
      db.run('DROP FUNCTION IF EXISTS prevent_aqlvoy_dish_delete()', (err) => {
        if (err) console.error('Migration error dropping Aqlvoy dish delete function:', err.message);
      });
