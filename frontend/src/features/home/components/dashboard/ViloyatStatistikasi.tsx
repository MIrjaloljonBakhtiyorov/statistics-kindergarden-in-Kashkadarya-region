...
import { COLORS, kindergartenImages, districts as fallbackDistricts, kindergartenTypes as fallbackKindergartenTypes } from '../../../../constants';
...
const TYPE_KEYS = ['type.davlat', 'type.xususiy', 'type.dxsh', 'type.oilaviy', 'type.tashkilot'];
const DAILY_CHILD_COST = 27600;
...
type KindergartenRow = {
  ...
  actualChildrenCount?: number | string;
  attendedBefore9?: number | string;
  attendedAfter9?: number | string;
  absent?: number | string;
};

type DistrictExpenseRow = {
  district?: string;
  costPerChild?: number | string;
  cost_per_child?: number | string;
};
...
type DashboardDistrict = {
  ...
  details: {
    ...
    types: Array<{ name: string; count: number; children: number }>;
  };
};
...
const normalizeDistrictKey = (value: unknown) => normalizeText(value)
  .replace(/\s+(tumani|tuman|shahri|shahar)$/g, '')
  .replace(/\s+(t\.|sh\.)$/g, '');
...
const childCountOf = (kg: KindergartenRow) => toNumber(kg.actualChildrenCount ?? kg.childrenCount);
const presentBeforeNineOf = (kg: KindergartenRow) => toNumber(kg.attendedBefore9);
const presentAfterNineOf = (kg: KindergartenRow) => toNumber(kg.attendedAfter9);
const presentCountOf = (kg: KindergartenRow) => presentBeforeNineOf(kg) + presentAfterNineOf(kg);
const absentCountOf = (kg: KindergartenRow) => {
  const explicitAbsent = toNumber(kg.absent);
  const derivedAbsent = childCountOf(kg) - presentCountOf(kg);
  return Math.max(explicitAbsent, derivedAbsent, 0);
};
const formatCount = (value: number) => Math.round(value).toLocaleString('uz-UZ');

const normalizeKindergartenType = (value: unknown) => {
  const text = normalizeText(value);
  if (text.includes('private') || text.includes('xususiy')) return 'type.xususiy';
  if (text.includes('dxsh') || text.includes('ppp') || text.includes('sherik')) return 'type.dxsh';
  if (text.includes('home') || text.includes('oilaviy')) return 'type.oilaviy';
  if (text.includes('tashkilot') || text.includes('department') || text.includes('organization')) return 'type.tashkilot';
  return 'type.davlat';
};

const fallbackAttendanceBreakdown = (children: number) => {
  const beforeNine = Math.round(children * 0.789);
  const afterNine = Math.round(children * 0.052);
  const absent = Math.max(0, children - beforeNine - afterNine);
  return { beforeNine, afterNine, absent };
};

const buildDistrictsFromRows = (rows: KindergartenRow[]): DashboardDistrict[] => {
  const districts = new Map<string, DashboardDistrict>();

  rows.forEach((kg) => {
    const name = String(kg.district || "Noma'lum hudud").trim() || "Noma'lum hudud";
    if (!districts.has(name)) {
      districts.set(name, {
        ...
        details: {
          ...
          types: TYPE_KEYS.map((typeName) => ({ name: typeName, count: 0, children: 0 })),
        },
      });
    }
    ...
    const beforeNine = presentBeforeNineOf(kg);
    const afterNine = presentAfterNineOf(kg);
    const absent = absentCountOf(kg);
    const typeName = normalizeKindergartenType(kg.type);
    const type = district.details.types.find((item) => item.name === typeName);

    district.count += 1;
    district.beforeNine += beforeNine;
    district.afterNine += afterNine;
    district.absent += absent;
    district.details.totalMTT += 1;
    district.details.totalChildren3to7 += children;
    district.details.totalCoveredChildren += children;
    if (type) {
      type.count += 1;
  ...
  return Array.from(districts.values())
    .map((district) => {
      const present = district.beforeNine + district.afterNine;
      const total = district.details.totalChildren3to7;
      district.attendance = total ? Math.round((present / total) * 10000) / 100 : 0;
      district.details.coveragePercentage = district.attendance;
      return district;
    })
    .sort((a, b) => a.name.localeCompare(b.name, 'uz'));
};

const fallbackDistrictsWithAttendance: DashboardDistrict[] = fallbackDistricts.map((district) => {
  const children = district.details?.totalChildren3to7 || 0;
  const breakdown = fallbackAttendanceBreakdown(children);
  return {
    ...district,
    beforeNine: breakdown.beforeNine,
    afterNine: breakdown.afterNine,
    absent: breakdown.absent,
  };
});

const renderActiveShape = (props: any, unit = 'ta') => {
  ...
  return (
    <g>
      ...
      <text x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} textAnchor={textAnchor} fill="#00008B" style={{ fontSize: '13px', fontWeight: 900 }}>{`${value} ${unit}`}</text>
...
const ViloyatStatistikasi: React.FC<ViloyatStatistikasiProps> = ({ setSelectedMTTType, CustomTooltip }) => {
  ...
  const [districtExpenses, setDistrictExpenses] = useState<DistrictExpenseRow[]>([]);
  const { t } = useTranslation();
  const presidentSlides = [
    {
      src: "https://www.uzreport.news/fotobank/image/cf2cfb0173dfb779fbec4bc5bcb76e48.jpeg",
      label: t('region.goal'),
      title: t('region.coverageGoalPrefix'),
      accent: `90%${t('region.coverageGoalSuffix')}`,
      badge: t('region.preschoolSystem'),
    },
    {
      src: "https://static.xabar.uz/crop/3/7/960__80_377301991.jpg",
      label: t('region.preschoolSystem'),
      title: t('region.eachChild'),
      accent: `${t('region.stateCare')} ${t('region.underCare')}`,
      badge: t('region.presidentVisit'),
    },
    {
      src: "https://data.daryo.uz/media/2019/06/cb53393f-5e8f-1bca-ec10-3f63a339d103.jpg",
      label: t('region.photoTalk'),
      title: t('region.photoTalk'),
      accent: t('region.preschoolSystem'),
      badge: t('region.presidentVisit'),
    },
    {
      src: "https://xabar.uz/static/crop/3/5/920__95_3585833392.jpg",
      label: t('region.photoModern'),
      title: t('region.photoModern'),
      accent: t('region.stateCare'),
      badge: t('region.preschoolSystem'),
    },
    {
      src: "https://static.xabar.uz/crop/2/4/736_736_95_2491735778.jpg",
      label: t('region.presidentVisit'),
      title: t('region.eachChild'),
      accent: t('region.aboutPreschool'),
      badge: t('region.preschoolSystem'),
    },
  ];
  ...
  useEffect(() => {
    ...
    kindergartenApi.getAll()
      .then((res) => {
        if (!mounted) return;
        setKindergartens(Array.isArray(res) ? res : []);
      })
      .catch(() => {
        if (!mounted) return;
        setKindergartens([]);
  ...
  useEffect(() => {
    let mounted = true;
    apiClient.get('/kindergartens/daily-district-expenses')
      .then((res) => {
        if (!mounted) return;
        const entries = Array.isArray(res.data?.entries) ? res.data.entries : [];
        setDistrictExpenses(entries);
      })
      .catch(() => {
        if (mounted) setDistrictExpenses([]);
      });
  ...
  const dashboardData = useMemo(() => {
    const hasLiveData = kindergartens.length > 0;
    const dashboardDistricts = hasLiveData ? buildDistrictsFromRows(kindergartens) : fallbackDistrictsWithAttendance;
    const typeMap = new Map(TYPE_KEYS.map((name) => [name, { name, count: 0, children: 0 }]));

    if (hasLiveData) {
      kindergartens.forEach((kg) => {
        const name = normalizeKindergartenType(kg.type);
        const type = typeMap.get(name)!;
        type.count += 1;
        type.children += childCountOf(kg);
      });
    } else {
      fallbackKindergartenTypes.forEach((type) => {
        typeMap.set(type.name, { ...type });
      });
    }

    const kindergartenTypes = TYPE_KEYS.map((name) => typeMap.get(name)!);
    const totalChildren = dashboardDistricts.reduce((acc, d) => acc + (d.details?.totalChildren3to7 || 0), 0);
    const totalCoveredChildren = dashboardDistricts.reduce((acc, d) => acc + (d.details?.totalCoveredChildren || 0), 0);
    const totalMTT = kindergartenTypes.reduce((acc, curr) => acc + curr.count, 0);
    const totalBeforeNine = dashboardDistricts.reduce((acc, d) => acc + d.beforeNine, 0);
    const totalAfterNine = dashboardDistricts.reduce((acc, d) => acc + d.afterNine, 0);
    const totalAbsent = dashboardDistricts.reduce((acc, d) => acc + d.absent, 0);
    const attendedChildren = totalBeforeNine + totalAfterNine;
    const coveragePercent = totalChildren ? (attendedChildren / totalChildren) * 100 : 0;
    const plannedDailyCost = totalChildren * DAILY_CHILD_COST;
    const totalRegionalDailyCost = attendedChildren * DAILY_CHILD_COST;
    const savedDailyCost = Math.max(0, totalAbsent * DAILY_CHILD_COST);
    const activeTypeCount = kindergartenTypes.filter((type) => type.count > 0).length || kindergartenTypes.length;

    return {
      dashboardDistricts,
      kindergartenTypes,
      totalChildren,
      totalCoveredChildren,
      totalMTT,
      totalBeforeNine,
      totalAfterNine,
      totalAbsent,
  ...
  const attendanceChartData = dashboardData.dashboardDistricts.map((d) => ({
    name: d.name,
    erta: d.beforeNine,
    kech: d.afterNine,
    kelmagan: d.absent,
  ...
  const expenseByDistrict = useMemo(() => {
    ...
    districtExpenses.forEach((entry) => {
      const key = normalizeDistrictKey(entry.district);
      const cost = toNumber(entry.costPerChild ?? entry.cost_per_child);
      if (key && cost > 0) map.set(key, cost);
    });
    return map;
  }, [districtExpenses]);

  const averageCostPerChild = useMemo(() => {
    const costs = districtExpenses
      .map((entry) => toNumber(entry.costPerChild ?? entry.cost_per_child))
      .filter((cost) => cost > 0);
    if (!costs.length) return DAILY_CHILD_COST;
    return costs.reduce((sum, cost) => sum + cost, 0) / costs.length;
  }, [districtExpenses]);

  const costForDistrict = (districtName: string) => expenseByDistrict.get(normalizeDistrictKey(districtName)) || 0;

  const costChartData = dashboardData.dashboardDistricts.map((d) => {
    const costPerChild = costForDistrict(d.name);
    const kelganCost = (d.beforeNine + d.afterNine) * costPerChild;
    const tejamkorlik = d.absent * costPerChild;
    return { name: d.name, kelganCost, tejamkorlik };
  });
  const plannedDailyCost = dashboardData.dashboardDistricts.reduce((sum, district) => {
    return sum + (district.details?.totalChildren3to7 || 0) * costForDistrict(district.name);
  }, 0);
  const totalRegionalDailyCost = costChartData.reduce((sum, item) => sum + item.kelganCost, 0);
  const savedDailyCost = costChartData.reduce((sum, item) => sum + item.tejamkorlik, 0);
  const costSpentPercent = plannedDailyCost ? (totalRegionalDailyCost / plannedDailyCost) * 100 : 0;
  const savedCostPercent = plannedDailyCost ? (savedDailyCost / plannedDailyCost) * 100 : 0;

  const districtAnalytics = dashboardData.dashboardDistricts.map((d) => {
    const totalChildren = d.details?.totalChildren3to7 || 0;
    const absent = d.absent;
    const profit = absent * costForDistrict(d.name);
    ...
    .sort((a, b) => (b.profit - a.profit) || (b.absent - a.absent))
    .map((item, index) => ({ ...item, rank: index + 1 }));

  const regionalTrend = ['14 May', '15 May', '16 May', '17 May', '18 May', '19 May', '20 May'].map((name, index) => {
    const multiplier = 0.68 + index * 0.053;
    return {
      name,
      profit: Number(((savedDailyCost * multiplier) / 1000000000).toFixed(2)),
    };
  });
          ...
          <div className="flex-1 flex flex-col lg:flex-row gap-8 items-center relative z-10">
              ...
              <ResponsiveContainer width="100%" height={320}>
                <PieChart>
                  <Pie
                    ...
                    data={fallbackKindergartenTypes}
                    innerRadius="60%" 
                    outerRadius="80%" 
                    paddingAngle={5} 
                    dataKey="count"
                  ...
                  >
                    {fallbackKindergartenTypes.map((_, index) => (
            ...
            <div className="flex-1 grid grid-cols-2 gap-3">
              {fallbackKindergartenTypes.map((type, index) => {
                ...
                const isLast = index === fallbackKindergartenTypes.length - 1;
                return (
                  <motion.div
                    key={index}
                    onClick={() => {
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className={`text-[20px] font-bold leading-none ${isActive ? 'text-white' : 'text-[#003580]'}`}>
                          {type.count.toLocaleString()} <span className="text-[14px]">{t('common.countUnit')}</span>
                        </p>
              ...
              <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-sm bg-white dark:bg-slate-950 dark:border-slate-800">
                ...
                <div className="px-8 pt-7 pb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex items-center gap-5 text-[12px] font-medium text-slate-500">
                    ...
                    <span className="flex items-center gap-2">
                      <span className="w-3 h-3 rounded-sm inline-block bg-[#F59E0B]" />{t('region.afterNineShort')}
                    </span>
                    <span className="flex items-center gap-2">
                      <span className="w-3 h-3 rounded-sm inline-block bg-[#EF4444]" />{t('region.absentShort')}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="h-[680px] w-full">
                    <ResponsiveContainer width="100%" height={600}>
                      ...
                      >
                        ...
                        />
                        <Bar dataKey="erta"     name={t('region.beforeNineShort')} fill="url(#blueGrad)"  stackId="keldi" radius={[0, 0, 2, 2]} barSize={18} />
                        <Bar dataKey="kech"     name={t('region.afterNineShort')} fill="url(#amberGrad)" stackId="keldi" radius={[4, 4, 0, 0]} barSize={18} />
                        <Bar dataKey="kelmagan" name={t('region.absentShort')} fill="url(#redGrad)"   radius={[4, 4, 2, 2]} barSize={18} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
               </div>
                  ...
                  <div className="relative h-80 shrink-0">
                    ...
                    <div className="absolute inset-0 p-6 flex flex-col justify-end gap-2">
                      ...
                      <h4 className="text-[22px] font-bold text-white leading-tight">
                        Shavkat Mirziyoyev - <span className="text-green-300">{t('region.aboutPreschool')}</span>
                      </h4>
                    </div>
                  </div>
                  ...
                  <div className="bg-white flex-1 flex flex-col gap-4 p-6">
                    ...
                    <div className="flex items-start gap-4 pb-3 border-b border-slate-100">
                      ...
                      <div className="flex flex-col justify-center gap-1">
                        ...
                        <p className="text-[11px] font-medium text-slate-500 leading-snug">{t('region.presidentRole')}</p>
                        <p className="text-[11px] font-medium text-slate-400">{t('region.since2016')}</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 pt-1">
                      {[
                        { label: t('region.covered'), value: formatCount(dashboardData.totalCoveredChildren) },
                        { label: t('stats.totalMTT'), value: formatCount(dashboardData.totalMTT) },
                        { label: t('region.coveragePercent'), value: `${dashboardData.coveragePercent.toFixed(1)}%` },
                      ].map((s, i) => (
              ...
              <div className="rounded-2xl overflow-hidden shadow-xl border border-slate-200">
                ...
                <div className="bg-[#001228] px-8 pt-8 pb-7">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[11px] font-semibold text-amber-300 uppercase tracking-widest">{t('region.financialAnalysis')}</span>
                  </div>
                  <h4 className="text-[22px] font-bold text-white leading-tight mb-6">
                    {t('region.dailyCostsByDistrict')}
                  </h4>
                    ...
                    <div className="rounded-xl p-5 relative overflow-hidden" style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.1)'}}>
                      ...
                      <div className="mt-3 h-1 rounded-full bg-white/10">
                        <div className="h-full rounded-full bg-[#4ADE80]"
                          style={{width:`${costSpentPercent.toFixed(0)}%`}} />
                      </div>
                      <p className="text-[10px] text-[#4ADE80] mt-1 font-semibold">
                        {costSpentPercent.toFixed(1)}% {t('region.spent')}
                      </p>
                    </div>
                    ...
                    <div className="rounded-xl p-5 relative overflow-hidden" style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.1)'}}>
                      ...
                      <div className="mt-3 h-1 rounded-full bg-white/10">
                        <div className="h-full rounded-full bg-amber-400"
                          style={{width:`${savedCostPercent.toFixed(0)}%`}} />
                      </div>
                      <p className="text-[10px] text-amber-300 mt-1 font-semibold">
                        {savedCostPercent.toFixed(1)}% {t('region.saved')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-5 mt-5 text-[12px] font-medium text-slate-400">
                    ...
                    <span className="flex items-center gap-2">
                      <span className="w-3 h-3 rounded-sm inline-block bg-amber-400" />{t('region.savings')}
                    </span>
                    <span className="text-[11px] text-slate-500 italic">{t('region.legendTotal')}</span>
                  </div>
                </div>
                                ...
                                <div className="bg-[#001228] text-white p-4 rounded-xl shadow-2xl min-w-[220px] space-y-2 border border-white/10">
                                  ...
                                  <div className="flex items-center justify-between gap-6">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-[11px] text-slate-400">{t('region.planned')}</span>
                                    </div>
                                  ...
                                  <div className="flex items-center justify-between gap-6">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-[11px] text-slate-400">{t('region.attendedChildren')}</span>
                                    </div>
                                  ...
                                  <div className="flex items-center justify-between gap-6 pt-1 border-t border-white/10">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-[11px] text-slate-400">{t('region.economy')}</span>
                                    </div>
                                  ...
                                  <p className="text-[10px] text-slate-400 text-right">{pct}% {t('region.spent')}</p>
                                </div>
                              );
                            }
                            return null;
                    ...
                    <div className="px-6 pt-7 pb-4 shrink-0 border-b border-slate-100 dark:border-slate-800">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <h4 className="text-[18px] font-black text-slate-900 leading-tight dark:text-white">{t('region.savedFunds')}</h4>
                        </div>
                        <div className="flex items-center gap-2 rounded-full bg-emerald-50 border border-emerald-100 px-3 py-1.5 dark:bg-emerald-500/15 dark:border-emerald-400/20">
                          <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                          <span className="text-[11px] font-bold text-emerald-700">{districtAnalytics.length} {t('footer.districts').toLowerCase()}</span>
                        </div>
                      </div>
                    </div>
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-center justify-between px-5 pt-5 shrink-0">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-[9px] font-bold uppercase tracking-[0.22em] text-emerald-400/60 leading-none mb-0.5">{t('region.province')}</p>
                          <p className="text-[11px] font-black uppercase tracking-[0.15em] text-emerald-300/90 leading-none">{t('region.totalProfit')}</p>
                        </div>
                      </div>
                    ...
                    <div className="relative z-10 px-5 pt-3 pb-3 shrink-0">
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-3">
                        {[
                          ...
                          { label: t('region.efficiency'), val: `${savedCostPercent.toFixed(1)}%`, accent: "#fbbf24", bg: "rgba(251,191,36,0.08)", br: "rgba(251,191,36,0.15)" },
                        ].map((k, i) => (
                                ...
                                return (
                                  <div style={{ background: "#071f14", border: "1px solid rgba(52,211,153,0.25)", borderRadius: 12, padding: "8px 13px", boxShadow: "0 16px 32px rgba(0,0,0,0.5)" }}>
                                    ...
                                    <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
                                      ...
                                      <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>{t('common.billion')}</span>
                                    </div>
                                  </div>
                                );
                              }
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-center justify-between px-5 pt-5 pb-4 shrink-0 border-b border-slate-100 dark:border-slate-800">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-indigo-500 leading-none mb-0.5">{t('region.aiSystem')}</p>
                          <p className="text-[15px] font-black text-slate-900 leading-tight tracking-tight dark:text-white">{t('region.monitoringForecast')}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-rose-50 border border-rose-100 dark:bg-rose-500/15 dark:border-rose-400/20">
                    ...
                    <div className="relative z-10 grid grid-cols-2 gap-3 p-4 flex-1 min-h-0">
                      {[
                        ...
                        { title: t('region.resourceForecast'), value: "15%", status: t('region.forecast'), icon: Lightbulb, accent: "#0369a1", bg: "#f0f9ff", br: "#bae6fd", ic: "#0ea5e9" },
                        { title: t('region.systemStability'), value: "99.9%", status: t('region.stable'), icon: Activity, accent: "#4338ca", bg: "#eef2ff", br: "#c7d2fe", ic: "#6366f1" },
                      ].map((item, idx) => (
                        <motion.div
                          key={idx}
