    ...
    <div className="w-full h-full flex flex-col" onMouseMove={handleMouseMove}>
      ...
      <div className="mb-6 flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-600 mb-1">{t('modal.interactiveMap')}</p>
            <h2 className="text-2xl font-black text-slate-900 tracking-tight leading-tight">
              Qashqadaryo - <span className="text-indigo-600">{t('modal.districtMap')}</span>
            </h2>
          </div>
        </div>
        <div className="flex gap-3 flex-wrap">
          {[
            { icon: MapPin,     label: t('footer.districts'),    value: 16,              color: '#6366f1', bg: '#eef2ff' },
            { icon: School,     label: t('stats.totalMTT'),    value: totalMTT,        color: '#10b981', bg: '#f0fdf4' },
            { icon: Activity,   label: t('modal.avgCoverage'), value: `${avgCoverage}%`, color: '#f59e0b', bg: '#fffbeb' },
            { icon: TrendingUp, label: t('modal.selected'),   value: selectedDistrict ?? '-', color: '#8b5cf6', bg: '#f5f3ff' },
          ].map((s, i) => (
