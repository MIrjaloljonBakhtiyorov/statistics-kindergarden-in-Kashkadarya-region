...
const CustomBarTooltip = ({ active, payload, label, unit }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#0f172a] text-white p-3 sm:p-4 rounded-2xl shadow-2xl border border-white/10 min-w-[132px] max-w-[190px]">
        <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 break-words">{label}</p>
        <p className="text-xl sm:text-2xl font-black tracking-tight">{payload[0].value} <span className="text-xs sm:text-sm font-bold text-slate-400">{unit}</span></p>
      </div>
    );
  }
  return null;
...
export default function MahsulotSarfi() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      {/* Product Tabs */}
      <div className="rounded-[1.35rem] sm:rounded-[2rem] border border-slate-100 bg-white/90 shadow-sm shadow-slate-100 p-3.5 sm:p-5 md:p-6 overflow-hidden dark:bg-slate-900/90 dark:border-slate-800">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-4 mb-5 sm:mb-6">
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            <div className="h-11 w-11 sm:h-14 sm:w-14 shrink-0 rounded-xl sm:rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center shadow-sm dark:bg-emerald-500/15 dark:border-emerald-400/20">
              <Leaf className="h-5 w-5 sm:h-6 sm:w-6 text-emerald-500" />
            </div>
            <div className="min-w-0">
              <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight dark:text-white">Mahsulotlar</h2>
              <p className="text-xs sm:text-sm font-semibold text-slate-400 leading-snug dark:text-slate-300">Omborxona markazidagi mahsulotlarni tanlang</p>
            </div>
          </div>

          <div className="flex w-full flex-col min-[520px]:flex-row lg:w-auto items-stretch min-[520px]:items-center gap-2">
            <div className="relative w-full lg:w-[320px]">
              ...
              <input
                ...
                className="w-full h-11 sm:h-12 rounded-xl sm:rounded-2xl border border-slate-200 bg-white pl-11 pr-4 text-sm font-bold text-slate-700 placeholder:text-slate-400 shadow-sm focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-300 dark:bg-slate-950 dark:border-slate-700 dark:text-white"
              />
            </div>
            <button
        ...
        <div
          ref={productRailRef}
          className="flex gap-2.5 sm:gap-3 overflow-x-auto pb-3 snap-x snap-mandatory custom-scrollbar"
        >
          {visibleProducts.map((p) => {
            ...
            return (
              <motion.button
                ...
                className={`relative h-[166px] w-[96px] sm:h-[188px] sm:w-[112px] md:w-[120px] shrink-0 snap-start rounded-[1.1rem] sm:rounded-[1.35rem] border transition-all duration-300 overflow-hidden group ${
                  isActive
              ...
              >
                ...
                <div className="h-full px-2.5 sm:px-3 py-3 sm:py-4 flex flex-col items-center justify-between text-center">
                  <p className={`text-[9px] sm:text-[10px] font-black uppercase tracking-[0.16em] sm:tracking-[0.18em] ${isActive ? 'text-white/75' : 'text-slate-400'}`}>{p.unit}</p>
                  <div className={`h-12 w-12 sm:h-16 sm:w-16 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all ${
                    ...
                    <p.icon className={`h-5 w-5 sm:h-7 sm:w-7 ${isActive ? 'text-white' : 'text-slate-500 group-hover:text-emerald-600 dark:text-slate-300 dark:group-hover:text-emerald-300'}`} />
                  </div>
                  <p className={`text-[10px] sm:text-[12px] font-black uppercase leading-[1.12] line-clamp-3 ${isActive ? 'text-white' : 'text-slate-900 dark:text-slate-100'}`}>{p.label}</p>
                </div>
              </motion.button>
            );
          })}
      ...
      {/* KPI Cards */}
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-2.5 sm:gap-4 items-stretch">
        {[
          {
          ...
          <motion.div
            ...
            className={`rounded-2xl sm:rounded-3xl p-3 sm:p-4 border flex flex-col justify-between min-h-[124px] sm:min-h-[112px] ${
              kpi.dark
          ...
          >
            <div className="flex items-start justify-between gap-2 mb-3 sm:mb-4">
              <p className={`min-w-0 text-[9px] sm:text-xs font-black uppercase tracking-[0.12em] sm:tracking-[0.18em] leading-snug ${kpi.dark ? 'text-slate-500' : 'text-slate-400'}`}>
                {kpi.label}
              </p>
              <div
            ...
            <div>
              <p
                className="text-[22px] sm:text-4xl xl:text-5xl font-black tracking-tight leading-none"
                style={{ color: kpi.dark ? '#ffffff' : (kpi.accent ?? '#0f172a') }}
              >
                {kpi.value}
              </p>
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1 mt-2">
                <span className={`text-[10px] sm:text-sm font-bold uppercase tracking-widest ${kpi.dark ? 'text-slate-500' : 'text-slate-400'}`}>
                  {kpi.unit}
                </span>
                {kpi.sub && (
                  <span className="min-w-0 text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-wider truncate">
                    — {kpi.sub}
                  </span>
                )}
              </div>
      ...
      <div className="rounded-[1.5rem] sm:rounded-[2rem] border border-slate-100 bg-white shadow-sm overflow-hidden dark:bg-slate-900 dark:border-slate-800">
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 px-4 sm:px-6 md:px-7 pt-5 sm:pt-6 pb-5 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            <div className="h-11 w-11 sm:h-12 sm:w-12 shrink-0 rounded-xl sm:rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center dark:bg-indigo-500/15 dark:border-indigo-400/20">
              <Archive className="h-5 w-5 text-indigo-600" />
            </div>
            <div className="min-w-0">
              <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.18em] sm:tracking-[0.22em] text-indigo-500 mb-1">Bir haftalik hisobot</p>
              <h3 className="text-base sm:text-xl font-black text-slate-900 tracking-tight dark:text-white">
          ...
          <label className="flex w-full sm:w-auto items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 dark:bg-slate-950 dark:border-slate-700">
            ...
            <div className="min-w-0 flex-1">
              ...
              <input
                ...
                className="w-full bg-transparent text-sm font-black text-slate-800 outline-none dark:text-white"
              />
            </div>
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.35fr] gap-0 divide-y lg:divide-y-0 lg:divide-x divide-slate-100 dark:divide-slate-800">
          <div className="p-3 sm:p-6 grid grid-cols-2 gap-2.5 sm:gap-3 content-start">
            {[
              ...
              <div key={item.label} className="rounded-xl sm:rounded-2xl border border-slate-100 bg-slate-50/60 p-3 sm:p-4 dark:bg-slate-950/70 dark:border-slate-800">
                <div className="flex items-start justify-between gap-2 mb-3 sm:mb-4">
                  <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-wider sm:tracking-widest text-slate-400 leading-snug">{item.label}</p>
                  <div className="h-7 w-7 sm:h-8 sm:w-8 shrink-0 rounded-lg sm:rounded-xl flex items-center justify-center" style={{ background: item.bg }}>
                    <item.icon className="h-4 w-4" style={{ color: item.color }} />
                  </div>
                </div>
                <p className="text-lg sm:text-2xl font-black tracking-tight text-slate-900 dark:text-white">{item.value}</p>
                <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-slate-400 mt-1 line-clamp-1">{item.sub}</p>
              </div>
            ))}
          </div>
          ...
          <div className="p-3 sm:p-5">
            <div className="max-h-[330px] overflow-y-auto custom-scrollbar space-y-2 pr-1">
              {weeklyProductTotals.map((product, index) => {
                ...
                return (
                  <div key={product.id} className="rounded-xl sm:rounded-2xl border border-slate-100 bg-white px-3 sm:px-4 py-3 hover:bg-slate-50 transition-colors dark:bg-slate-950/80 dark:border-slate-800 dark:hover:bg-slate-800/80">
                    <div className="flex items-center justify-between gap-3 sm:gap-4 mb-2">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs sm:text-sm font-black uppercase tracking-tight text-slate-900 truncate dark:text-white">{product.label}</p>
                          <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{product.districts} hudud</p>
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-sm sm:text-base font-black text-[#003580]">{Math.round(product.quantity).toLocaleString('uz-UZ')}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Chart Panel */}
        <div className="lg:col-span-8 bg-white rounded-[1.5rem] sm:rounded-3xl border border-slate-100 shadow-sm overflow-hidden dark:bg-slate-900 dark:border-slate-800">
          <div className="p-4 sm:p-6 lg:p-8 pb-0">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6 sm:mb-8">
              <div>
                <h3 className="text-lg sm:text-2xl font-black text-slate-900 tracking-tight uppercase leading-tight dark:text-white">
              ...
              {/* Chart type toggle */}
              <div className="flex w-full sm:w-auto items-center gap-1 bg-slate-50 p-1 rounded-xl border border-slate-100 dark:bg-slate-950 dark:border-slate-800">
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="px-2 sm:px-4 pb-6 sm:pb-8"
            >
              <ResponsiveContainer width="100%" height={isCompact ? 360 : 340} minWidth={0} minHeight={0}>
                {chartType === 'bar' ? (
                  <BarChart data={chartData} margin={isCompact ? { top: 10, right: 6, left: -24, bottom: 96 } : { top: 10, right: 10, left: -20, bottom: 82 }} barCategoryGap={isCompact ? "14%" : "22%"}>
                    <defs>
                    ...
                    <XAxis
                      ...
                      tickLine={false}
                      interval={0}
                      angle={isCompact ? -55 : -40}
                      textAnchor="end"
                      tick={{ fill: '#64748b', fontSize: isCompact ? 9 : 10, fontWeight: 800 }}
                      height={isCompact ? 96 : 82}
                    />
                    <YAxis
                      axisLine={false}
                      tickLine={false}
                      tick={{ fill: '#94a3b8', fontSize: isCompact ? 9 : 10, fontWeight: 700 }}
                    />
                    <Tooltip content={<CustomBarTooltip unit={activeUnit} />} cursor={{ fill: 'rgba(148, 163, 184, 0.12)', radius: 8 }} wrapperStyle={{ outline: 'none' }} />
                    <Bar dataKey="quantity" fill={activeProduct.color} radius={[8, 8, 4, 4]} barSize={isCompact ? 18 : 28}>
                      {chartData.map((_, i) => (
                  ...
                  <AreaChart data={chartData} margin={isCompact ? { top: 10, right: 6, left: -24, bottom: 96 } : { top: 10, right: 10, left: -20, bottom: 82 }}>
                    <defs>
                    ...
                    <YAxis
                      ...
                      tick={{ fill: '#94a3b8', fontSize: isCompact ? 9 : 10, fontWeight: 700 }}
                    />
                    <Tooltip content={<CustomAreaTooltip unit={activeUnit} />} cursor={{ stroke: activeProduct.color, strokeWidth: 1 }} wrapperStyle={{ outline: 'none' }} />
                    <Area
                      type="monotone"
                      dataKey="quantity"
        ...
        {/* Ranking Sidebar */}
        <div className="lg:col-span-4 bg-white rounded-[1.5rem] sm:rounded-3xl border border-slate-100 shadow-sm flex flex-col dark:bg-slate-900 dark:border-slate-800">
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-1.5 max-h-[380px] sm:max-h-[440px]">
              ...
              {filteredData.map((d, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`p-3 sm:p-3.5 rounded-2xl transition-all group cursor-pointer ${
                      isTop
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 mb-2">
                      <div className="flex min-w-0 items-center gap-2.5">
                        ...
                        <span className={`font-black uppercase tracking-tight text-[11px] truncate ${isTop ? 'text-white' : 'text-slate-800 dark:text-slate-100'}`}>
                          {districtName(d.name)}
                        </span>
                      </div>
                      <span className={`shrink-0 font-black text-xs sm:text-sm ${isTop ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
      ...
      {/* Bottom Summary Row */}
      <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-2.5 sm:gap-4">
        {productTotals.map((p, i) => {
          ...
          return (
            <motion.button
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.07 }}
              className={`p-3 sm:p-5 rounded-2xl sm:rounded-3xl border text-left transition-all duration-300 ${
                isActive
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className={`min-w-0 text-[8px] sm:text-[9px] font-black uppercase tracking-wider sm:tracking-widest line-clamp-2 ${isActive ? 'text-white/70' : 'text-slate-400'}`}>{p.label}</span>
              </div>
              <p className={`text-lg sm:text-xl font-black tracking-tight ${isActive ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
