...
import { getDistrictNameKey, productData, products } from '../../../constants';
...
export default function MahsulotSarfi() {
  ...
  const activeUnit = t(activeProduct.unitKey ?? activeProduct.unit);
          ...
          return (
            ...
            >
              ...
              <div className="text-left">
                <p className={`text-[9px] font-bold uppercase tracking-widest leading-none mb-0.5 ${isActive ? 'text-white/70' : 'text-slate-400'}`}>{t(p.unitKey ?? p.unit)}</p>
                <p className={`text-sm font-black uppercase tracking-tight leading-none ${isActive ? 'text-white' : 'text-slate-900'}`}>{t(p.labelKey ?? p.label)}</p>
              </div>
            </motion.button>
          );
        })}
            ...
            <div className="flex items-start justify-between gap-4 mb-8">
              <div>
                <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase">
                  {t(activeProduct.labelKey ?? activeProduct.label)} <span className="text-indigo-600">{t('product.consumption')}</span>
                </h3>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <ResponsiveContainer width="100%" height={320} minWidth={0} minHeight={0}>
                {chartType === 'bar' ? (
                  <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 20 }} barCategoryGap="30%">
                    <defs>
                    ...
                    />
                    <Tooltip content={<CustomBarTooltip unit={activeUnit} />} cursor={{ fill: '#f8fafc', radius: 8 }} />
        ...
        <div className="lg:col-span-4 bg-white rounded-3xl border border-slate-100 shadow-sm flex flex-col">
          <div className="p-6 border-b border-slate-50">
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-base font-black text-slate-900 uppercase tracking-tight">{t('footer.districts')}</h3>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className={`text-[9px] font-black uppercase tracking-widest ${isActive ? 'text-white/70' : 'text-slate-400'}`}>{t(p.labelKey ?? p.label)}</span>
              </div>
              ...
              <p className={`text-[9px] font-bold uppercase tracking-widest mt-0.5 ${isActive ? 'text-white/60' : 'text-slate-400'}`}>
                {t('stats.total')} {t(p.unitKey ?? p.unit)}
              </p>
            </motion.button>
          );
        })}
