...
const JARIMALAR = [
  { id: 'taomnoma', labelKey: 'rating.penaltyMenu', ball: 20, icon: UtensilsCrossed, color: '#dc2626', grad: 'from-red-600 to-red-700' },
  { id: 'gigiyena', labelKey: 'rating.penaltyHygiene', ball: 15, icon: Droplets, color: '#ea580c', grad: 'from-orange-600 to-orange-700' },
  { id: 'sanitariya', labelKey: 'rating.penaltySanitary', ball: 10, icon: Shield, color: '#ca8a04', grad: 'from-yellow-500 to-yellow-700' },
  { id: 'operatsion', labelKey: 'rating.penaltyOperational', ball: 5, icon: ClipboardX, color: '#7c3aed', grad: 'from-violet-600 to-violet-800' },
  { id: 'davomat', labelKey: 'rating.penaltyAttendance', ball: 20, icon: Activity, color: '#1d4ed8', grad: 'from-blue-700 to-indigo-700' },
  { id: 'foto', labelKey: 'rating.penaltyPhoto', ball: 5, icon: Camera, color: '#db2777', grad: 'from-pink-600 to-pink-700' },
];

const MTT_TURLARI = [
  { id: 'barchasi', labelKey: 'common.all' },
  { id: 'davlat', labelKey: 'type.shortDavlat' },
  { id: 'nodavlat', labelKey: 'type.shortNodavlat' },
  { id: 'dxsh', labelKey: 'type.shortDxsh' },
  { id: 'oilaviy', labelKey: 'type.shortOilaviy' },
  { id: 'tashkilot', labelKey: 'type.shortTashkilot' },
];
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 bg-[#0f172a] rounded-2xl px-5 py-3.5">
          ...
          <div>
            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{t('rating.formula')}</p>
            <p className="text-sm font-black text-white leading-none">100 x 31 {t('rating.day')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
        {JARIMALAR.map((j, i) => (
          ...
          >
            ...
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{t(j.labelKey)}</p>
      ...
      <div className="bg-white border border-slate-100 rounded-3xl px-6 py-5 shadow-sm flex flex-wrap items-center gap-6">
        ...
        <div>
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('rating.mttType')}</p>
          <div className="flex gap-1.5 flex-wrap">
            {MTT_TURLARI.map(type => (
              <button
                key={type.id}
                onClick={() => setSelectedTur(type.id)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-8 pt-8 pb-0">
            ...
            <p className="text-[13px] font-bold text-slate-400 uppercase tracking-widest mt-1 mb-6">
              {isEmpty ? '0' : sorted.length} {t('common.countUnit')} {activeHudud ? 'MTT' : t('common.region').toLowerCase()} / Max: {MAX_BALL.toLocaleString()} {t('rating.score')}
            </p>
          </div>
          ...
          {isEmpty ? (
            <div className="h-72 flex flex-col items-center justify-center gap-4 px-8 pb-8">
              ...
              <div className="text-center">
                <p className="text-sm font-black text-slate-500 uppercase tracking-widest">{t('common.noData')}</p>
                <p className="text-[10px] font-bold text-slate-300 uppercase tracking-wider mt-1">
                  {activeHudud} {t('rating.emptyPrefix')} {t(MTT_TURLARI.find(type => type.id === selectedTur)?.labelKey ?? 'common.all')} MTT {t('rating.emptySuffix')}
                </p>
              </div>
            </div>
          ) : (
        ...
        <div className="lg:col-span-5 bg-white rounded-3xl border border-slate-100 shadow-sm flex flex-col overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-50 flex items-center justify-between">
            <div>
              <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">{t('rating.table')}</h3>
              <p className="text-[13px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                {activeHudud ?? t('common.all')} / {t(MTT_TURLARI.find(type => type.id === selectedTur)?.labelKey ?? 'common.all')}
              </p>
            </div>
            <button
      ...
      {/* в”Ђв”Ђ MUSOBAQALAR VA FAOLIYAT в”Ђв”Ђ */}
      <div className="space-y-6 mt-4">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">
                {t('rating.competitions')} & <span className="text-amber-500">{t('rating.activity')}</span>
              </h3>
              <p className="text-[13px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                {t('rating.activitySubtitle')}
              </p>
            </div>
          </div>
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-2.5 rounded-xl px-4 py-2.5" style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)' }}>
                  ...
                  <span className="text-[13px] font-black text-white">2025 {t('rating.yearPlanned')}</span>
                </div>
                <div className="flex items-center gap-2.5 rounded-xl px-4 py-2.5" style={{ background: 'rgba(251,191,36,0.12)', border: '1px solid rgba(251,191,36,0.25)' }}>
                  ...
                  <span className="text-[13px] font-black text-amber-300">{t('rating.awaitingApproval')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: t('rating.plannedChecks'), value: "1,280+", icon: CheckCircle2, color: "#6ee7b7", glow: "#10b981" },
                { label: t('rating.prizeFund'), value: "500 mln", icon: Award, color: "#f9a8d4", glow: "#ec4899" },
              ].map((s, i) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0, y: 12 }}
        ...
        <div className="rounded-[2rem] border border-slate-100 shadow-sm p-8" style={{ background: 'linear-gradient(135deg, #fafafa 0%, #f8faff 100%)' }}>
          <div className="flex items-center gap-3 mb-7">
            ...
            <div>
              <h4 className="text-[18px] font-black text-slate-900 uppercase tracking-tight">{t('rating.stages')}</h4>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">{t('rating.stagesSubtitle')}</p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              { step: "01", title: t('rating.stageScore'), desc: t('rating.stageScoreDesc'), icon: Activity, color: "#6366f1", light: "#eef2ff" },
              { step: "02", title: t('rating.stageDistrict'), desc: t('rating.stageDistrictDesc'), icon: Star, color: "#0ea5e9", light: "#f0f9ff" },
              { step: "03", title: t('rating.stageRegion'), desc: t('rating.stageRegionDesc'), icon: Trophy, color: "#f59e0b", light: "#fffbeb" },
              { step: "04", title: t('rating.stageAward'), desc: t('rating.stageAwardDesc'), icon: Award, color: "#10b981", light: "#ecfdf5" },
            ].map((item, i) => (
              <motion.div
                key={i}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { place: t('rating.place1'), gol: t('rating.notChosen'), tuman: t('rating.districtNotApproved'), medal: "1", color: "#f59e0b", shadow: "shadow-amber-100", border: "border-amber-200", bg: "from-amber-50 via-orange-50 to-white" },
            { place: t('rating.place2'), gol: t('rating.notChosen'), tuman: t('rating.districtNotApproved'), medal: "2", color: "#6366f1", shadow: "shadow-indigo-100", border: "border-indigo-200", bg: "from-indigo-50 via-purple-50 to-white" },
            { place: t('rating.place3'), gol: t('rating.notChosen'), tuman: t('rating.districtNotApproved'), medal: "3", color: "#10b981", shadow: "shadow-emerald-100", border: "border-emerald-200", bg: "from-emerald-50 via-teal-50 to-white" },
          ].map((item, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 12 }}
