            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ padding: '20px 20px 0' }}>
                  <button onClick={onClose} style={{
                    ...
                    <span style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.15em' }}>{t('taomnoma.close')}</span>
                  </button>
                </div>
                ...
                <div style={{ padding: '24px 20px 0', flex: 1 }}>
                  ...
                  <div style={{ marginTop: 28, display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {[
                      { label: t('modal.children3to7'), value: details.totalChildren3to7.toLocaleString(), color: '#60a5fa' },
                      { label: t('modal.covered'), value: details.totalCoveredChildren.toLocaleString(), color: '#34d399' },
                      { label: t('modal.mttInstitutions'), value: details.totalMTT, color: '#a78bfa' },
                    ].map((m, i) => (
                ...
                <div style={{ padding: '20px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ fontSize: 8, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>{t('modal.coverageLevel')}</span>
              ...
              <div style={{ flex: 1, background: '#f8fafc', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '20px 28px', background: '#fff', borderBottom: '1px solid #eef0f5', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                  <div>
                    <h3 style={{ fontSize: 18, fontWeight: 900, color: '#0f172a', letterSpacing: '-0.02em' }}>{t('modal.detailedAnalysis')}</h3>
                    <p style={{ fontSize: 10, color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.15em', marginTop: 2 }}>{t('modal.regionalReport')}</p>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10 }}>
                    ...
                    <span style={{ fontSize: 9, fontWeight: 800, color: '#16a34a', textTransform: 'uppercase', letterSpacing: '0.15em' }}>{t('modal.liveData')}</span>
                  </div>
                </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }} className="custom-scrollbar">
                  {hasDetails ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                          ...
                          <h4 style={{ fontSize: 12, fontWeight: 900, color: '#0f172a', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{t('modal.mainIndicators')}</h4>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                          {[
                            { label: t('modal.age3to7'), value: details.totalChildren3to7, suffix: t('common.childrenShort'), icon: Baby, accent: '#6366f1', bg: '#eef2ff', br: '#e0e7ff' },
                            { label: t('modal.covered'), value: details.totalCoveredChildren, suffix: t('common.childrenShort'), icon: Users, accent: '#10b981', bg: '#f0fdf4', br: '#bbf7d0' },
                            { label: t('modal.institutions'), value: details.totalMTT, suffix: t('common.countUnit'), icon: School, accent: '#8b5cf6', bg: '#faf5ff', br: '#e9d5ff' },
                          ].map((s, i) => (
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                          ...
                          <h4 style={{ fontSize: 12, fontWeight: 900, color: '#0f172a', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{t('modal.mttTypes')}</h4>
                        </div>

                        {/* table header */}
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 90px', padding: '0 14px 8px', gap: 8 }}>
                          {[t('modal.typeName'), t('modal.count'), t('modal.children')].map(h => (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 16 }}>
                      ...
                      <p style={{ fontSize: 16, fontWeight: 900, color: '#1e293b', textTransform: 'uppercase', letterSpacing: '-0.01em' }}>{t('modal.dataFilling')}</p>
                      <p style={{ fontSize: 12, color: '#94a3b8', maxWidth: 260, textAlign: 'center' }}>{t('modal.dataSoon')}</p>
                    </div>
                  )}
                </div>
                ...
                <div style={{ padding: '16px 28px', background: '#fff', borderTop: '1px solid #eef0f5', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontSize: 9, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.15em' }}>{t('modal.dataUpdated')}</span>
                  </div>
                  <button onClick={onClose} style={{
