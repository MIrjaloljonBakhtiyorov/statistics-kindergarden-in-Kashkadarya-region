...
const MTTDetailModal: React.FC<MTTDetailModalProps> = ({ selectedMTTType, setSelectedMTTType, modalRef }) => {
  ...
  const typeIndex = selectedMTTType ? kindergartenTypes.findIndex(t => t.name === selectedMTTType.name) : -1;
  const typeColor = typeIndex !== -1 ? COLORS[typeIndex % COLORS.length] : '#6366f1';
  const features = selectedMTTType?.features ?? [
    `${t('modal.mttInstitutions')}: ${selectedMTTType?.count?.toLocaleString?.() ?? 0}`,
    `${t('modal.children')}: ${selectedMTTType?.children?.toLocaleString?.() ?? 0}`,
    t('modal.regionalMonitoring'),
  ];

  return (
    <AnimatePresence>
      {selectedMTTType && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 md:p-12 pointer-events-none">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <motion.div 
            ...
            className="relative bg-white w-[52vw] max-w-[min(900px,calc(100vw-2rem))] max-h-[70vh] rounded-none shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] overflow-hidden pointer-events-auto border border-white/20 flex flex-col"
          >
            <div className="h-40 shrink-0 sticky top-0 z-30 flex items-end p-8 lg:p-10 overflow-hidden" style={{ background: typeColor }}>
              <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-32 -mt-32 blur-3xl" />
              <div className="absolute top-4 right-4 lg:top-6 lg:right-6 z-40">
                <button 
                  onClick={() => setSelectedMTTType(null)}
              ...
              <div className="relative z-10">
                <h3 className="text-3xl lg:text-4xl font-black text-white tracking-tighter uppercase leading-none">{t(selectedMTTType.name)}</h3>
                <p className="text-white/80 font-bold text-[10px] uppercase tracking-[0.3em] mt-3">{t('modal.detailInfoAnalysis')}</p>
              </div>
            </div>
            ...
            <div className="p-8 lg:p-10 space-y-8 overflow-y-auto flex-1 custom-scrollbar">
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-slate-50 p-6 lg:p-8 rounded-[2rem] border border-slate-100 flex flex-col justify-between group hover:border-indigo-100 transition-colors">
                  ...
                  <div>
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('modal.institutions')}</p>
                ...
                <div className="bg-slate-50 p-6 lg:p-8 rounded-[2rem] border border-slate-100 flex flex-col justify-between group hover:border-indigo-100 transition-colors">
                  ...
                  <div>
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('modal.pupils')}</p>
              ...
              <div className="space-y-3">
                ...
                <p className="text-slate-500 leading-relaxed font-bold text-sm bg-slate-50/50 p-6 rounded-2xl border border-slate-100 italic">
                  "{selectedMTTType.description ? t(selectedMTTType.description) : t('modal.orgDescriptionFallback')}"
                </p>
              </div>
