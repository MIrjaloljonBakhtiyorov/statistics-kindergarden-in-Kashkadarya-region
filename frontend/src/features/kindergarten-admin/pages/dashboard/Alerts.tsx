...
const STATUS_CONFIG = {
  success: { color: 'text-emerald-500', bg: 'bg-emerald-50', dot: 'bg-emerald-500', icon: CheckCircle2 },
  update: { color: 'text-blue-500', bg: 'bg-blue-50', dot: 'bg-blue-500', icon: RefreshCw },
  warning: { color: 'text-amber-500', bg: 'bg-amber-50', dot: 'bg-amber-500', icon: AlertTriangle },
  error: { color: 'text-rose-500', bg: 'bg-rose-50', dot: 'bg-rose-500', icon: AlertCircle },
  ai: {
...
export const Alerts = () => {
  ...
  const filteredAlerts = useMemo(() => alerts.filter(alert => {
    ...
    const matchesSearch =
      ...
      alert.context.toLowerCase().includes(query);
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between bg-white/50 backdrop-blur-md p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-white shadow-sm sticky top-0 z-30 gap-4">
        <div>
          ...
          <p className="text-[10px] sm:text-xs font-bold text-slate-500">
            Bu sahifa faqat real tizim hodisalari ulangandan keyin xabarlarni ko'rsatadi
          </p>
        </div>
          ...
          <div className="space-y-6 relative">
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col items-center">
                      <div className={clsx("w-6 h-6 rounded-full border-4 border-slate-50 relative z-10 flex items-center justify-center transition-transform group-hover:scale-110 shadow-sm", config.dot)} />
                      <div className="mt-2 text-[10px] font-black text-slate-400 uppercase tracking-tighter whitespace-nowrap">{alert.time}</div>
                    </div>
                    <div className={clsx("flex-1 p-5 rounded-[2rem] border transition-all duration-300 hover:shadow-xl hover:-translate-y-1 relative overflow-hidden", isAI ? config.bg + ' ' + config.special : "bg-white border-slate-100 shadow-sm")}>
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex gap-4">
                          <div className={clsx("w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-inner", isAI ? "bg-indigo-500/20 text-indigo-400" : config.bg + ' ' + config.color)}>
                            <Icon size={22} />
                          </div>
                          <div>
                            <h3 className={clsx("text-sm font-black mb-1", isAI ? "text-white" : "text-slate-900")}>{alert.title}</h3>
                            <span className={clsx("text-[11px] font-bold", isAI ? "text-indigo-300" : "text-slate-500")}>{alert.context}</span>
                            <div className="flex items-center gap-4 mt-3">
                              <div className="flex items-center gap-1.5">
                                <div className={clsx("w-5 h-5 rounded-full flex items-center justify-center", isAI ? "bg-white/10" : "bg-slate-100")}>
                                  <User size={10} className={isAI ? "text-indigo-300" : "text-slate-500"} />
                                </div>
                                <span className={clsx("text-[10px] font-black uppercase tracking-wider", isAI ? "text-slate-400" : "text-slate-500")}>{alert.actor}</span>
                              </div>
                        ...
                        <button className={clsx("p-2 rounded-xl transition-colors", isAI ? "text-slate-500 hover:bg-white/10" : "text-slate-300 hover:bg-slate-50 hover:text-slate-600")}>
                      ...
                      {isAI && (
                        <div className="mt-4 pt-4 border-t border-white/5 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 italic">AI tavsiyasi real modul ulangandan keyin shakllanadi</span>
                          </div>
                        </div>
                      )}
                    </div>
            ...
            {filteredAlerts.length === 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-20 text-center">
                ...
                <h3 className="font-black text-slate-400 uppercase tracking-widest text-sm">Hozircha real alertlar yo'q</h3>
                <p className="text-xs text-slate-400 mt-2 font-bold">Fake ogohlantirishlar olib tashlandi</p>
              </motion.div>
            )}
          </div>
        </div>
