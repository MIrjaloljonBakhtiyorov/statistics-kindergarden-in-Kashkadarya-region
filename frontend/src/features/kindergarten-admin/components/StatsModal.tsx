...
export const StatsModal = ({ isOpen, onClose, type, data = [] }: { isOpen: boolean, onClose: () => void, type: string | null, data?: any[] }) => {
  ...
  const rows = data.map((kg) => {
    const totalChildren = childCountOf(kg);
    const present = toNumber(kg.attendedBefore9) + toNumber(kg.attendedAfter9);
    return {
      id: kg.id,
      name: kg.name || "Noma'lum bog'cha",
      type: kg.type || '-',
      district: kg.district || "Noma'lum tuman",
      director: kg.directorName || '-',
      totalChildren,
      attendedBefore9: toNumber(kg.attendedBefore9),
      absent: toNumber(kg.absent),
      attendancePercentage: toNumber(kg.attendancePercentage) || attendancePercent(present, totalChildren),
    };
  });

  const districts = ['Barchasi', ...Array.from(new Set(rows.map(k => k.district)))];
