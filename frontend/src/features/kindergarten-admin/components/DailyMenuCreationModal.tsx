...
const savedRecipes: Array<{ id: number; name: string; cat: string }> = [];
...
export const DailyMenuCreationModal: React.FC<DailyMenuCreationModalProps> = ({ isOpen, onClose, selectedDate }) => {
  ...
  const filteredDB = savedRecipes.filter(item => item.name.toLowerCase().includes(searchQuery.toLowerCase()));
