...
import { Search, MapPin, CheckCircle, Download, Eye, FileUp, Video, Link as LinkIcon, GitBranch } from "lucide-react";
...
export function RegionsPage() {
  ...
  const [regionApplications, setRegionApplications] = useState<RegionApplication[]>([]);
  const [selectedApplication, setSelectedApplication] = useState<RegionApplication | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true);
        setLoadError("");
        const response = await fetch(`${API_BASE_URL}/admin/regions-summary`);
        const json = await response.json();
        if (!response.ok) throw new Error(json?.error?.message ?? "Hududlar yuklanmadi");
        setRegions(json.data ?? []);
      } catch (error) {
        console.error("Hududlar yuklanishda xatolik:", error);
        setRegions([]);
        setLoadError("Hududlar ma'lumotini backenddan yuklab bo'lmadi.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const stats = [
    { label: "Jami hududlar", value: regions.length, icon: MapPin, color: "text-blue-400" },
    { label: "Shaharlar", value: regions.filter((r) => r.type === "city").length, icon: CheckCircle, color: "text-green-400" },
    { label: "Tumanlar", value: regions.filter((r) => r.type === "district").length, icon: MapPin, color: "text-purple-400" },
    { label: "Jami arizalar", value: regions.reduce((s, r) => s + r.applicationsCount, 0), icon: CheckCircle, color: "text-cyan-400" },
  ];

  const handleViewApplications = async (region: Region) => {
    setSelectedRegion(region);
    setIsApplicationsLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/admin/user-applications?groupType=region&groupValue=${encodeURIComponent(region.name)}`);
      const json = await response.json();
      if (!response.ok) throw new Error(json?.error?.message ?? "Arizalar yuklanmadi");
      setRegionApplications(json.data ?? []);
    } catch (error) {
      console.error("Hudud arizalari yuklanishda xatolik:", error);
      setRegionApplications([]);
    } finally {
  ...
  return (
    <AdminShell>
      <AdminPageHeader
        title="Hududlar"
        subtitle="Hududlar kesimida kelib tushgan real arizalar"
        actions={
      ...
      <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl p-6 mb-6">
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          >
            ...
            <option value="city">Shaharlar</option>
            <option value="district">Tumanlar</option>
          </select>
        </div>
      </div>
      ...
      <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-[var(--admin-bg)] border-b border-[var(--admin-border)]">
              <tr>
                <th className="text-left py-4 px-6 text-sm font-medium text-[var(--admin-text-secondary)]">Hudud</th>
            ...
            <tbody>
              {filteredRegions.map((region) => (
                <tr key={region.id} className="border-b border-[var(--admin-border)] hover:bg-[var(--admin-bg)] transition-colors">
                  <td className="py-4 px-6"><div className="font-medium text-white">{region.name}</div></td>
                  <td className="py-4 px-6">
                    <span className={`text-xs font-medium ${region.type === "city" ? "text-cyan-400" : "text-purple-400"}`}>
                      {region.type === "city" ? "Shahar" : "Tuman"}
                    </span>
                  </td>
                  <td className="py-4 px-6"><span className="font-semibold text-white">{region.applicationsCount}</span></td>
                  <td className="py-4 px-6"><span className="font-semibold text-green-300">{region.acceptedCount}</span></td>
                  <td className="py-4 px-6 text-sm text-[var(--admin-text-secondary)]">{formatDate(region.lastApplicationAt)}</td>
                  <td className="py-4 px-6">
                    <div className="flex items-center justify-end">
                      <button onClick={() => handleViewApplications(region)} className="p-2 hover:bg-[var(--admin-surface-2)] rounded-lg transition-colors" title="Arizalarni ko'rish"><Eye size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {isLoading ? (
          <div className="py-12 text-center text-[var(--admin-text-muted)]">Hududlar yuklanmoqda...</div>
        ) : filteredRegions.length === 0 && (
          <div className="py-12 text-center text-[var(--admin-text-muted)]">
            <MapPin size={48} className="mx-auto mb-4 opacity-50" /><p>Hududlar bo'yicha real ariza hali yo'q</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-2xl max-w-5xl w-full max-h-[88vh] overflow-y-auto">
            <div className="sticky top-0 bg-[var(--admin-surface)] border-b border-[var(--admin-border)] px-6 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-[var(--admin-text-secondary)]">{regionApplications.length} ta ariza</p>
              </div>
              <button onClick={() => setSelectedRegion(null)} className="p-2 hover:bg-[var(--admin-bg)] rounded-lg">✕</button>
            </div>
            <div className="p-6 space-y-4">
              {isApplicationsLoading ? (
                <p className="text-center text-[var(--admin-text-muted)] py-8">Arizalar yuklanmoqda...</p>
              ) : regionApplications.length === 0 ? (
                <p className="text-center text-[var(--admin-text-muted)] py-8">Bu hudud bo'yicha ariza topilmadi</p>
              ) : regionApplications.map((app) => {
                ...
                return (
                  <div key={app.id} className="bg-[var(--admin-bg)] border border-[var(--admin-border)] rounded-xl p-5">
                    ...
                    <div className="mt-4 grid gap-2 text-sm text-[var(--admin-text-secondary)] sm:grid-cols-2 lg:grid-cols-4">
                      ...
                      <span>Viloyat: {app.region || "-"}</span>
                      <span>Hudud: {app.district || selectedRegion.name}</span>
                    </div>
...
function ApplicationDetailModal({ app, onClose }: { app: RegionApplication; onClose: () => void }) {
  const status = applicationStatusConfig[app.status] ?? applicationStatusConfig.submitted;
      ...
      <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Info label="Viloyat" value={app.region} />
            <Info label="Hudud" value={app.district} />
          </div>
