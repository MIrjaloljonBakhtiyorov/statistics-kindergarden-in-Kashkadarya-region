...
import { ImageOff, Utensils, ZoomIn } from 'lucide-react';
...
export const FoodImagePreview = ({
  ...
  if (!imageSrc || hasError) {
    return (
      <span
        className={clsx(
          'relative block overflow-hidden bg-gradient-to-br from-emerald-50 via-white to-amber-50 outline-none',
          className
        )}
      >
        <span className="absolute inset-0 opacity-70 [background:radial-gradient(circle_at_20%_18%,rgba(16,185,129,0.18),transparent_32%),radial-gradient(circle_at_80%_82%,rgba(245,158,11,0.18),transparent_36%)]" />
        <span className="relative flex h-full min-h-[92px] w-full flex-col items-center justify-center gap-2 p-4 text-center">
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/85 text-brand-primary shadow-sm ring-1 ring-emerald-100">
            {imageSrc ? <ImageOff size={22} /> : <Utensils size={23} />}
          </span>
          <span className="line-clamp-2 text-[10px] font-black uppercase tracking-widest text-brand-muted">
            {label || alt || "Taom rasmi"}
          </span>
        </span>
      </span>
    );
